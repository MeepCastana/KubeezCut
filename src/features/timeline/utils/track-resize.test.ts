...
describe('track-resize', () => {
  ...
  it('updates only the requested track and preserves unchanged arrays', () => {
    ...
    const resizedTracks = resizeTrackInList(tracks, 'a1', 108);

    expect(resizedTracks).not.toBe(tracks);
    expect(resizedTracks[0]).toBe(tracks[0]);
    expect(resizedTracks[1]).toMatchObject({ id: 'a1', height: 108 });
    expect(resizeTrackInList(tracks, 'missing', 108)).toBe(tracks);
  ...
  it('gives the full pane to video when there is no audio track (OpenCut-style single stack)', () => {
    const viewportHeight = 420;
    const trackTitleBarHeight = 24;
    const tracks = [createTrack('v1', 'video', 72)];
    ...
    expect(layout.hasTrackSections).toBe(false);
    expect(layout.availablePaneHeight).toBe(420);
    expect(layout.videoPaneHeight).toBe(420);
    expect(layout.audioPaneHeight).toBe(0);
  });

  it('reserves the full pane for an empty timeline (drop surface before any tracks)', () => {
    ...
    expect(layout.hasTrackSections).toBe(false);
    expect(layout.availablePaneHeight).toBe(360);
    expect(layout.videoPaneHeight).toBe(360);
    expect(layout.audioPaneHeight).toBe(0);
  });
